...
export default function ServiceTabs() {
  ...
  return (
    <div className='relative flex flex-col items-center w-full h-full px-3 overflow-hidden bg-black sm:bg-secondaryBrand'>
      ...
      <img src="icons/circles1.svg" alt="" className="animate-spin-slow hidden h-[30rem] w-[30rem] absolute -right-24 bottom-60 md:block" />

      <div className='z-10 flex flex-col w-full max-w-6xl gap-10 py-20 border-t-2 md:gap-20 md:py-28 border-t-black/20'>
        <header className="flex flex-col gap-5">
          <h1 className='flex-initial text-white sm:text-black text-[2.6rem] leading-[1] sm:leading-10 md:text-6xl lg:text-7xl font-helveticaNowText'>Como podemos ajudar?</h1>
          <section className="flex flex-wrap items-center gap-3 mt-10">
            {services.map(serviceButton => (
              <button 
                ...
                className={
                  "flex items-center justify-center lg:w-auto px-4 py-[0.35rem] lg:py-4 text-sm lg:text-[16px] font-matterRegular leading-4 rounded-full h-[2.75rem] lg:h-auto bg-brand duration-200 gap-2 "
        ...
        <div className='relative flex flex-col flex-auto gap-20 lg:gap-32'>
          <section className="bg-[#e2e0d6] w-full h-auto sm:h-[33rem] rounded-2xl md:rounded-[2.8rem] flex flex-col gap-3 md:gap-0 md:grid md:grid-cols-3 p-4 md:p-10">
            <figure ref={tilt} style={{ transition: 'box-shadow 0.1s, transform 0.1s' }} className="w-full h-[25vh] md:h-auto lg:h-full col-span-2 bg-[#cbc9c0] overflow-hidden rounded-lg md:rounded-3xl block my-0 mx-auto hover:cursor-pointer">
              <img src={service.image} alt={service.name} className='object-cover object-top w-full h-full' />
            </figure>
            <figcaption className="flex flex-col justify-center gap-2 sm:gap-5 md:px-5">
              <h3 className="text-2xl md:text-4xl font-matterSemiBold">{service.name}</h3>
